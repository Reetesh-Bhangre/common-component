import { Directive, ViewContainerRef, ElementRef, HostListener, forwardRef, ChangeDetectorRef, OnInit, OnChanges, SimpleChanges, Input, DoCheck, KeyValueDiffer, KeyValueDiffers, Output, EventEmitter, Renderer2, HostBinding, ComponentFactoryResolver, } from '@angular/core';
...
export class DaterangepickerDirective implements OnInit, OnChanges, DoCheck {
  ...
  @Input() defaultRangeLabel: string = this.locale.customRangeLabel;
  ...
  ngOnInit() {
    this.dataService.$dateRangeReset
      .pipe(takeUntil(this.destroy$))
      .subscribe(value => {
      if (value) {
        this.dateRangeReset(value);
      }
    });
    ...
    // Setting up default range label manually
    // With this user can set the range label on page load
    if (this.ranges) {
      setTimeout(() => {
        if (this.defaultRangeLabel !== this.locale.customRangeLabel) {
          this.picker.defaultRangeLabel = this.defaultRangeLabel;
          this.picker.chosenRange = this.defaultRangeLabel;
          this.picker.clickRange('', this.defaultRangeLabel);
  ...
  /** Reset dates to its original state */
  dateRangeReset(value: string) {
    this.defaultRangeLabel = value;
    this.picker.clickRange('', this.defaultRangeLabel);
    this.picker.clickApply();
    this._changeDetectorRef.markForCheck();
  }
  ...
  inputChanged(e) {
    ...
    let start = null,
      ...
      start = format(new Date(dateString[0]), this.picker.locale.format);
      end = format(new Date(dateString[1]), this.picker.locale.format);
      }
      if (this.singleDatePicker || start === null || end === null) {
      start = format(new Date(e.target.value), this.picker.locale.format);
        end = start;
      }
